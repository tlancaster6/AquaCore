...
must_haves:
  ...
  artifacts:
    - path: "src/aquakit/refraction.py"
    ...
    - path: "src/aquakit/triangulation.py"
  ...
  key_links:
    - from: "src/aquakit/refraction.py"
      to: "src/aquakit/interface.py"
...
Import from aquakit: `InterfaceParams` from types, `ray_plane_intersection` from interface.
...
Import from aquakit: none needed (pure torch).
...
Run `python -c "from aquakit import snells_law_3d, triangulate_rays, refractive_project"` — imports succeed.
  </verify>
  <done>
