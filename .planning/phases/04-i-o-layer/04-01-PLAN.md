...
must_haves:
  ...
  artifacts:
    - path: "src/aquakit/io/frameset.py"
...
<task type="auto">
  ...
  <action>
**frameset.py** — Implement the FrameSet runtime_checkable Protocol following the exact pattern from `src/aquakit/projection/protocol.py`:
...
Do NOT update the top-level `aquakit/__init__.py` yet — that happens in Plan 02 when all exports are ready.
  </action>
  <verify>
Run `hatch run typecheck` — frameset.py and images.py must pass basedpyright.
Run `python -c "from aquakit.io import FrameSet, ImageSet; print('OK')"` — imports succeed.
  </verify>
  <done>
FrameSet Protocol is defined with all 5 required methods. ImageSet class handles construction, validation, indexing, iteration, and context manager. Both are importable from aquakit.io.
  </done>
</task>
