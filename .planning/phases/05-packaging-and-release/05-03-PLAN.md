...
must_haves:
  truths:
    - "Rewiring guide lists every AquaCal import with its AquaKit replacement"
    - "Rewiring guide lists every AquaMVS import with its AquaKit replacement"
  ...
  key_links:
    - from: ".planning/rewiring/REWIRING.md"
      to: "src/aquakit/__init__.py"
      via: "all new imports must be valid aquakit exports"
      pattern: "from aquakit import"
---

<objective>
Write the rewiring guide that maps every old AquaCal and AquaMVS import to its AquaKit replacement.

Purpose: Consumer teams (AquaCal, AquaMVS) need a find-and-replace reference to migrate their imports. The guide also documents intentional gaps (modules not ported to AquaKit).
...
<task type="auto">
  <name>Task 1: Verify AquaKit public API against research mapping</name>
  <files></files>
  <action>
1. Read `src/aquakit/__init__.py` and collect all exports from `__all__`.
...
5. Read `src/aquakit/camera.py` to verify `create_camera` signature for the guide (research open question #6).
...
## Prerequisites
- Install PyTorch separately (aquakit does not declare torch as a dependency)
- Install aquakit: `pip install aquakit`
...
- Depth of examples: Claude's discretion based on actual API differences
  ...
  <verify>File exists at `.planning/rewiring/REWIRING.md`; every export in `aquakit.__all__` appears at least once in the guide; both AquaCal and AquaMVS sections present; NOT PORTED sections present</verify>
...
- Every item in `aquakit.__all__` appears in at least one import table
