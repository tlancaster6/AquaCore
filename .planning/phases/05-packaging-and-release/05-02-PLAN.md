...
user_setup:
  ...
  - service: pypi
    ...
    dashboard_config:
      - task: "Create aquakit project on PyPI (if not exists)"
...
5. Go to https://pypi.org (create `aquakit` project if needed).
6. In project settings > Publishing > Add trusted publisher: Owner=`tlancaster6`, Repository=`aquakit`, Workflow=`publish.yml`, Environment=`pypi`.
