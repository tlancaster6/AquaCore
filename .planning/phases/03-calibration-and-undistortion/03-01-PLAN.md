...
must_haves:
  truths:
    ...
    - "aquakit is importable with AquaCal uninstalled (no AquaCal import anywhere)"
    - "Bad camera entries are skipped with a warning, not a crash"
  artifacts:
    - path: "src/aquakit/calibration.py"
  ...
  key_links:
    - from: "src/aquakit/calibration.py"
...
<task type="auto">
  ...
  <files>src/aquakit/calibration.py, src/aquakit/__init__.py</files>
  <action>
...
- Add to __all__: "CalibrationData", "CameraData", "load_calibration_data"
  ...
  <verify>
Run `hatch run typecheck` to confirm type correctness. Run `python -c "from aquakit import CalibrationData, CameraData, load_calibration_data; print('OK')"` to confirm imports work.
  </verify>
  <done>
calibration.py contains CameraData, CalibrationData, and load_calibration_data with no AquaCal imports. All three are importable from aquakit. Type checker passes.
  </done>
</task>
...
- `python -c "from aquakit import load_calibration_data, CalibrationData, CameraData"` -- imports work
- No `aquacal` or `from aquacal` appears anywhere in src/aquakit/calibration.py
</verification>
