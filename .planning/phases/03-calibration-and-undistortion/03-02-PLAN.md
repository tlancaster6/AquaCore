...
must_haves:
  ...
  artifacts:
    - path: "src/aquakit/undistortion.py"
    ...
    - path: "tests/unit/test_undistortion.py"
      ...
      contains: "def test_"
  key_links:
    - from: "src/aquakit/undistortion.py"
...
<task type="auto">
  ...
  <files>src/aquakit/undistortion.py, src/aquakit/__init__.py</files>
  <action>
...
- Add to __all__: "compute_undistortion_maps", "undistort_image"
  ...
  <verify>
Run `hatch run typecheck` to confirm type correctness. Run `python -c "from aquakit import compute_undistortion_maps, undistort_image; print('OK')"` to confirm imports work.
  </verify>
  <done>
undistortion.py contains both functions. compute_undistortion_maps dispatches pinhole/fisheye correctly. undistort_image handles PyTorch tensor round-trip. Both importable from aquakit.
  </done>
</task>
...
- `python -c "from aquakit import compute_undistortion_maps, undistort_image"` -- imports work
</verification>
